...
def tokenization(line, data, line_number,labels):
    ...
    # B Type Instruction
        ...
        if is_number(label):
            ...
            offset=label//2
            if label%4!=0:
                print("The third token is expected to be a divisible of 4")
                return None
        else:
        ...
        # Finding equivalent immediate value
            offset=(star_line-line_number)*4
        # Report Error if the immediate is not within the acceptable range
...
# Taking input in the text file 
f=open("input.txt","w")
print("Please enter your assembly code. Press Enter without typing anything to finish.")
while True:
   line=input()
   if not line:
       break
